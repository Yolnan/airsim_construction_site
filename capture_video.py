import sys
sys.path.append("./AirSim/PythonClient/multirotor")
sys.path.append("./data-driven-exploration/capture_point_cloud_unreal_engine")

import setup_path 
from capture_pointcloud_camera_frame import *
import airsim
import os
import math
import time
...
class OrbitNavigator:
    def __init__(self, radius = 2, altitude = 5, speed = 2, iterations = 1, center = [1,0], snapshots = None):
        self.radius = radius
...
#%%
    def customized_orbit(self):
        ...
        while count < self.iterations:
            ...
            ## move
            print(self.home.z_val,self.altitude)

            ###########################################################################
            # take image
            # camera_id = "0"
            # obot_name = "Drone2"
            # get pointcloud with rgb color
            # img_rgb, img_depth = get_RGBD(client, camera_id, robot_name)

            # get pointcloud with semantic color
            # img_rgb, img_depth = get_SemanticDepth(client, camera_id, robot_name)
            ##########################################################################

            pose = self.client.simGetVehiclePose("Drone2")
            print("pose ",pose)
            ...
            camera_pose = airsim.Pose(airsim.Vector3r(0, 0, 0), airsim.to_quaternion(-math.pi/8, 0, math.pi/6))  #PRY in radians
            self.client.simSetCameraPose("0", camera_pose);

            for i in range(0,50,2):
                self.client.moveToPositionAsync(5, i, z, 2).join()
                img_rgb, img_depth = get_SemanticDepth(client, camera_id, robot_name)
                # write to file
                cv2.imwrite("img_rgb"+str(i)+".png", img_rgb)
                cv2.imwrite("image_depth"+str(i)+".png",img_depth)
                #np.savez('rgbd3.npz', rgb=img_rgb, depth=img_depth)

            self.client.moveToPositionAsync(5, 50, z, 2).join()
            print("move to ",-40, -40, z)
            
            
            self.client.moveToPositionAsync(20, 50, z, 2).join()
            print("move to ",-5, -10, z)

            self.client.moveToPositionAsync(20, 20, z, 2).join()
            print("move to ",-5, -5, z)
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--altitude", type=float, help="altitude of orbit (in positive meters)", default=8)
